...
public class YamlConfigUpdater {
    ...
    public void update(YamlConfiguration config, File configFile) {
        ...
        try{
            ...
            newVersion = new Version("1.0.5");
            if (curVersion.compareTo(newVersion) < 0){
                curVersion = updateTo1Point05(configFile,backupDir, curVersion, newVersion);}
        } catch (Exception e){
            e.printStackTrace();
        }
    }
    ...
    private Version updateTo1Point05(File oldFile, File backupDir, Version newVersion, Version oldVersion) throws IOException {
        FileUpdater fu = new FileUpdater(oldFile,backupDir,newVersion,oldVersion);
        fu.replace(".*version:.*", "version: 1.0.5");
        fu.addAfter(".*sendPVEDeathMessages:.*", "",
                "useMobDisplayNames: false # use custom mob names if they exist, instead of generic (Skeleton, Zombie, Spider, etc)");
        return fu.update();
    }
}
